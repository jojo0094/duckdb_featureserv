...
##      PROGRAM - Name of binary, duckdb_featureserv
...
$(PROGRAM): $(GOFILES) 
	go build -v -ldflags "-s -w -X github.com/tobilg/duckdb_featureserv/conf.setVersion=$(APPVERSION)"
...
# to be used in docker the built binary needs the CGO_ENABLED=0 option
	CGO_ENABLED=0 go build -v -ldflags "-s -w -X github.com/tobilg/duckdb_featureserv/conf.setVersion=$(APPVERSION)"
